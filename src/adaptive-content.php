...
function custom_category_logger() {
    ...
    wp_enqueue_script('custom-category-logger', plugins_url('/js/logger.js', __FILE__), ['model_prompting', 'google_topics'], '1.0', true);

    // Pass data to the logger script.
    wp_localize_script('model_prompting', 'my_ajax_object', ['ajax_url' => admin_url( 'admin-ajax.php' )]);
    wp_localize_script('custom-category-logger', 'postCategory', $category_name);
    wp_localize_script('custom-category-logger', 'blockData', $block_data);
    wp_localize_script('custom-category-logger', 'postID', $post_id);
    wp_localize_script('custom-category-logger', 'cat_views', $_SESSION['category_views']);
}
